...
try:
    ...
    while True:
        ...
        if stop_signal == 1:
            ...
            error=cl_L.value()-cl_R.value()
            integral+=error
            derivative = error -prev_error
            mv = KP * error + KI * integral + KD * derivative
            prev_error = error
            lspeed = BASE_SPEED - mv
            rspeed = BASE_SPEED + mv
            if (lspeed > 100): 
                lspeed = 100
            if (lspeed < -100): 
                lspeed = -100
            ...
            mR.duty_cycle_sp = -rspeed
            mL.duty_cycle_sp = -lspeed
            mB.duty_cycle_sp = -max(rspeed,lspeed)
