...
# -*- coding: utf-8 -*-
"""
Center-Line PD Follower (corrected)
- The black line is between both sensors.
- This version computes error = right - left so the robot CENTERS
  the line (does not evade it).
- Adds dt for D term, exponential filtering, and clamped corrections.
...
KP = 18        # Proportional gain Best Scenario = 18
KD = 4         # Derivative gain (uses dt) Best Scenario = 3.5
BASE_SPEED = 25   # Nominal base forward speed Best Scenario = 25
MAX_SPEED = 80    # Absolute maximum allowed speed per wheel Best Scenario = 80
MIN_SPEED = 10    # Minimum allowed speed when near line (turning) Best Scenario = 10

# Thresholds for line detection
LINE_THRESHOLD = 5        # generic white/black midpoint (calibrate to your surface) Best Scenario = 17
BOTH_BLACK_THRESHOLD = 3  # both sensors below this => both on black (intersection) Best Scenario = 10
RECOVERY_STRAIGHT_TIME = 1.0  # seconds to go straight before search
SEARCH_SPEED = 30          # speed used when pivoting to search

# Control extras
CORRECTION_FILTER_ALPHA = 0.65  # smoothing of correction (0..1)
MAX_CORRECTION = 18.0           # maximum correction magnitude applied to wheels
DEAD_ZONE = 1.0                 # small zone around zero error -> go straight

# Sampling
SAMPLE_INTERVAL = 0.02

# ===== SENSORS & MOTORS =====
cl_L = ColorSensor('in4')  # left
cl_R = ColorSensor('in1')  # right
try:
...
try:
    ...
    print("=== CENTER-LINE PD FOLLOWER (FIXED) ===")
    ...
    while not stop_signal:
        ...
        if running and not stop_signal:
            ...
            # send to motors (SpeedPercent expects -100..100)
            ...
            print("L:{:3d} R:{:3d} | Avg:{:4.1f} | Err:{:6.2f} | ML:{:5.1f} MR:{:5.1f} | State: {}".format(
                int(left_value), int(right_value), avg, error, left_speed, right_speed, pd_controller.recovery_state))
