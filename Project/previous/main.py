...
class RobotConfig:
    ...
    COLORS = ('unknown', 'black', 'blue', 'green', 'yellow', 'red',
              'white', 'brown')  # In case we use the color sensors
    # can be used to check if black is the color returned by the sensor COLORS_CHECK.get('black')==self.colorSensor.value()
    COLOR_CHECK = {color: position for position, color in enumerate(COLORS)}
    ...
    # Controls for how long to follow the line, is either follow_forever or follow_for_ms and then specify a variable ms=something
...
class LineFollowingRobot:
    ...
    def start(self):
        ...
        while not self.button.down():  # Waits to for this to be pressed before starting
            sleep(0.1)
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Lego Mindstorms Line Following Robot")
    parser.add_argument("--l_w", type=str, default=RobotConfig.LEFT_WHEEL,
                        help="Left wheel motor port (outA, outB, outC, outD)")
    parser.add_argument("--r_w", type=str, default=RobotConfig.RIGHT_WHEEL,
                        help="Right wheel motor port (outA, outB, outC, outD)")
    parser.add_argument("--sp", type=int, default=RobotConfig.WORKING_SPEED,
                        help="Working speed percentage (0-100)")
    ...
    parser.add_argument("--b", type=int, default=RobotConfig.BLACK,
                        help="Intensity for the line(0-255)")
    parser.add_argument("--w", type=int, default=RobotConfig.WHITE,
                        help="Intensity value for outside the line (0-255)")
    parser.add_argument("--follow_left_edge", type=bool,
                        default=RobotConfig.FOLLOW_LEFT_EDGE, help="True if sensor is on the left side")
    parser.add_argument("--follow_time", type=int, default=RobotConfig.FOLLOW_LINE_TIME,
                        help="Time in ms to follow line before doing other actions")
    parser.add_argument("--silent", "--s", action="store_true",
                        help="The robot won't beep ")
